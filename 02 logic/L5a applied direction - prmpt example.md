...
Full-screen, scroll-driven fashion/archive landing page for the brand **"prmpt"**. Two main phases:
...
- Height dynamically calculated (initially `500vh`, then overridden by GSAP to `vh + maxScroll + 2*vh`)
...
- `fixed`, `pointer-events-none`, `z-index: 50` — follows `mousemove`
- Positioned via direct DOM manipulation (`style.left/top = clientX/clientY`)
- `transform: translate(-50%, -50%)` to center on pointer
- `mix-blend-mode: exclusion`
- Contains a 48×48 SVG: circle with stroke (r=22.75, strokeWidth=2.5) + custom Japanese/decorative glyph path, all filled white

### 1B. Logo (Top Left)

- `position: fixed`, `pointer-events-none`, `z-index: 20`, `mix-blend-mode: exclusion`
...
- Motion animation: opacity 0→1, y 12→0, duration 0.6s, ease `[0.25, 0.1, 0.25, 1]`, delay 0s
- SVG viewBox `0 0 355 110`, "prmpt" wordmark + circled "R" mark, all paths filled white
...
- Font: Inter Tight 500, 12px, line-height 140%, letter-spacing -0.04em, color #FFFFFF
- Motion animation: same as logo, delay 0.3s
...
- `position: fixed`, `z-index: 20`, `pointer-events-none`, `mix-blend-mode: exclusion`
- Position: `top: 32px, right: 32px` (desktop) / `top: 16px, right: 16px` (mobile)
- Width: 330px (desktop), auto (mobile). Height: 30px
...
- Contents:
  ...
  - Flex row, gap 50px (desktop) / 20px (mobile):
    - Hamburger SVG: viewBox `0 0 40 40`, two lines (`M0 14H40`, `M0 26H40`), stroke white, strokeWidth 2.5. Size 30×30 (desktop), 24×24 (mobile)
    - "[ CART ]" text: Inter Tight 500, 15px (desktop) / 13px (mobile), white

### 1E. Product Info (Bottom Right)

- `id="outro-info"`, `position: fixed`, `pointer-events-none`, `z-index: 20`, `mix-blend-mode: exclusion`
...
- `data-outro-offset`: 166 (desktop), 132 (mobile)
- Contents:
  - Top block (flex-column, align flex-start, width 100% desktop / 252px mobile, margin-bottom 32px / 12px):
    - Circle icon: 30×30 (desktop) / 20×20 (mobile)
      - SVG circle (cx=20, cy=20, r=18.75, stroke white, strokeWidth 2.5 / 2)
      - `<span id="circle-symbol">` centered inside, starts "8", randomizes from `['8', '$', '^^', '%', '/']` on scroll (throttled 80ms)
      - Font: Inter Tight 500, 15px / 10px, letter-spacing -0.04em, uppercase, white
    - Collection label: Inter Tight 500, 30px / 20px, line-height 100%, text-align center, letter-spacing -0.04em, uppercase, white — `ARCHIVE COLLECTION` + line break + `"PROMPT"`
  - Price: Inter Tight 500, 80px / 60px, line-height 100%, text-align center, letter-spacing -0.04em, white — `$97,33`

### 1F. "View" Button (Bottom Right, Initially Hidden)

- `id="outro-buy"`, `position: fixed`, `pointer-events-none`, `z-index: 20`, `mix-blend-mode: exclusion`
- **Desktop**: right: 32px, bottom: 32px, width: 330px, height: 174px
- **Mobile**: left: 16px, right: 16px, bottom: 60px, height: 100px
- `transform-origin: right bottom`, `transform: scale(0)` — scales to 1 via scroll
- Background: #fff, border-radius: 1335px (pill), flex center
- Text "view": Inter Tight 500, 110px / 72px, letter-spacing -0.04em, color #fff, `mix-blend-mode: exclusion`
...
- Opacity transition 0→1 when both videos loaded (`opacity 0.3s ease`), `overflow: hidden`
- Two `<video>` elements: muted, playsInline, preload="auto", absolutely positioned, `object-fit: cover`
- Left starts `display: none`, right starts `display: block`

### 1H. Video Interaction Logic

**Desktop (non-touch):**
- Videos are NOT auto-played — scrubbed by cursor X position via `requestAnimationFrame`
- Dead zone: `Math.max(30, width * 0.05)` pixels from center
- In dead zone → keep current video at `currentTime = 0`
- Cursor left of dead zone → show RIGHT video, scrub by distance from center-left-edge to left edge
- Cursor right of dead zone → show LEFT video, scrub by distance from center+deadZone to right edge
- `activeSideRef` tracks last active side, only changes when cursor exceeds dead zone
- Progress: `(distance from dead zone edge) / (available range)` mapped to `0...video.duration`
- **CRITICAL**: Only update `currentTime` when `!video.seeking` — prevents jitter by waiting for browser to finish rendering the previous seek

**Mobile/Tablet (touch):**
- Videos auto-play alternately: left plays first, on `ended` switches to right, on right `ended` switches back to left
...
- Font: Inter Tight 500, 13px / 11px, letter-spacing -0.02em, uppercase, white
...
- Initially `translateY(100vh)` — slides up to `translateY(0)` during first 100vh of scroll via GSAP ScrollTrigger (scrub: true, ease: none)
...
- `buildLayout(count, cols)` per row `r`:
  - Primary column: `a = (r * 2 + (r % 2)) % cols`
  - Place one image at column `a`
  - Every 3rd row (`r % 3 === 0`): place second image at `b = (a + 2) % cols` (or `(a+1)%cols` if same as `a`)
  - Empty cells → `-1` (rendered as spacer divs)

### Card Behavior

- Class `bp-card`, `will-change: transform`, initially `transform: scale(0)`
- `transform-origin`: left-half cards → `right bottom`; right-half cards → `left bottom`
- Scale computed per-frame in RAF:
  - **Enter**: `Math.min(1, (vh - top) / (vh * 0.6))` — 0→1 as card enters viewport
  - **Exit**: `Math.min(1, bottom / (vh * 0.4))` — 1→0 as card exits top
  - Final: `Math.min(enter, exit)`
  - Fully off-screen → `scale(0)`

### Scroll Phases (RAF-based, not scroll events)

- **Phase 1** (scrollY 0 → vh): Panel slides up. Cards computed with `panelOffset = vh - scrollY`
- **Phase 2** (scrollY > vh): Panel fixed at top. Inner wrapper: `translateY(-(scrollY - vh))`. Cards recomputed with phase 2 offset.
- **Outro** (scrollY > vh + maxScroll): White overlay fades in, product info slides up by `outroOffset` px, "view" button scales 0→1, footer fades in. Progress: `(scrollY - vh - maxScroll) / (vh - 100)`
...
- All text overlays use `mix-blend-mode: exclusion` to stay visible against both light and dark backgrounds
- No visible scrollbar interaction — entirely RAF-driven position tracking
