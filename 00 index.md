...
- **Locked vs open controls** — the round-one mechanic that still governs expressions: commentary on a reference covers only 4–6 of a composition's controls; those get locked as keywords/terms/phrases; knowledge and media controls fill from the actual knowledge and media; everything else stays open, and cycling the open controls is what makes variations. Two rules added 7.12: a lock is never absolute or one-directional unless Buck's words were — hedges stay hedged, and negatives record only things Buck killed. And filling an open control follows this order: the folder's own refs → Buck's documented taste across the other expression docs (docs cite each other as evidence freely; that is not a constraint) → 01 settled + 06 knowledge → Buck's live direction in session, which supersedes stale doc text and must be written back into the doc the same session, version-bumped → the agent's own judgment last, flagged as such so a reaction to it isn't mistaken for a reaction to evidence.
- **Scenario** (unsettled name, Buck's word, 7.12) — where one expression's meaning depends on the state of another (e.g. 01's giant script reads differently depending on whether the wordmark stayed put or moved toward script), variations get built as explicit scenario-worlds — same mockup, different world — and judged as worlds against each other, not as isolated specimens. A doc that carries scenarios names them in their own section, with any build dependencies (an asset that must exist before a scenario can be built at all). Otherwise, variations are judged against their own expression doc only; cross-expression mentions are context and evidence, never constraints.
